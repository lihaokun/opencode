...
### P5 面板布局定稿（用户复验反馈，2026-09-25）

用户要求：去圆点（primary 高亮已表达当前会话）；列表最左侧与上一行目录路径最左侧
对齐；列表与该行拉开行距。 Option 的内置 paddingLeft 3 使标签列最低在容器+3，无法
达到路径的字形列（容器+1）——面板因此**定稿为刻意独立的紧凑布局**（不再复用
Option）：`marginTop 1`（行距）+ `paddingLeft 1`（左缘=路径字形列）+ 极简行
（indent+label+blurb 左、仪表右）+ 无圆点（isCurrent → 名称 primary）。设计文档
§5.2.3 的"复用 Option"表述由本条取代；"同一视觉行不允许两份渲染代码"的规则收敛为
"**同一观感**的行不允许两份"——面板是刻意不同的观感。
