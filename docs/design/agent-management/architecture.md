...
前置条件（Requires）：
  ...
  - Session 的 parentID 链无环（由 Session 创建路径保证，见 §7 H3）
...
不变式（Invariants）：
  - 一个 Agent 处理完当前消息序列后只交付一个最终结果；中间 assistant 消息留在 transcript
    （调研 §5.3；由既有单 output 槽语义保证，本架构不引入 per-message 结果承诺）
...
H1: running 是进程内真相。SessionStatus 存于 InstanceState，进程重启后清空。
    ...
    — 来源：调研 §6「不包含进程崩溃后的自动继续执行」；本架构不提供崩溃恢复，故不修复该退化

H2: 同一 Agent 任一时刻至多一个活动执行。
    — 来源：既有 Session 运行状态机（同一 Session 的第二次运行请求会加入现有执行而非并行开一个）

H3: Session 的 parentID 链无环且深度有限。
    — 来源：子 Session 只在创建时绑定 parentID 且此后不变；深度另有上限约束

H4: 终止通知的接收方在收到通知时仍在运行（除非它自己也在停止集内）。
    — 来源：I1 的排序保证；若该假设被破坏，通知会唤醒一个已停止的 Agent

H5: 一个 Agent 处理完当前消息序列后只交付一个最终结果。
    — 来源：调研 §10；既有单 output 槽语义。本架构不引入 per-message 结果承诺，故不需要 correlation ID

H6: 停止范围内不会有新成员在拆解过程中被派生。
    — 来源：本架构未做保证。已知既有实现按一次快照展开子树，拆解期间新派生的后代不在集合内。
      这是上游既有缺陷（调研 §10 已归入实现阶段独立验证），本架构不掩盖它，见 §8
```
...
I3: 单活动执行
    ...
    维护方：既有 Session 运行状态机（本架构复用，不重新实现）
    preservation：M3 对 running 目标不新起 run，而是让消息加入现有执行；
      M4 的取消以 Session 为单位，不产生并行执行。
```
...
1. **拆解期间新派生的后代不在停止集内**（H6）。既有子树展开按一次快照进行。
