...
4. **`task` 工具的删除尚未落地，卡在这批测试上**（实现阶段实测的结果，非预估）。
   四个 `opencode run` 子进程用例与两个 prompt 用例断言的是**前台**语义：子 Agent 失败时
   父的 tool part 转为 `error` 并携带子的失败正文。恒为异步后这条路径不存在——
   父的 agent part 在委托注册完成时即 `completed`，失败作为通知消息稍后到达。
   观察点必须从 tool 返回值移到通知消息，而这批用例没有等待通知的结构，
   表层改断言只会让它们变成偶发通过。
   **现状**：四个 Agent 工具已上线并对模型可见，`task` 的权限键已规范化为 `agent`，
   运行时与 UI 消费者已全部迁移，两个默认拒绝点已移除，深度默认已提到 3。
   剩下的只是把 `task` 从注册表摘掉并删除模块——需先围绕通知重写那六个用例，属独立一步。
