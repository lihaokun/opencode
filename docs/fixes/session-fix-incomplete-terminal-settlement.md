...
> 状态：P1-A / P1-B 已实施；current-HEAD focused、跨层与全量回归全部通过
...
| 回归 P1-A consumer | 模拟 run consumer 只输出 `time.end` text 并在 idle 停止，断言 `final partial` 在 idle 前可见 | event-timeline regression 已锁定 `text:partial-3 → assistant-terminal → idle`（`49a969975`）；真实 subprocess 全文件 20/20 通过 |
| 回归 P1-B | 强化现有 rollback-removal-failure：process 返回 stop、request count=1、durable `UnknownError("rollback removal failed")`、finish=error、time.completed、status=idle、无 summary | 已强化并通过：`session.processor does not request or summarize again when rollback removal fails`（`57e749efe`） |
| 新增交叉 | rollback preparation pure/mixed interrupt 仍不 replay，MessageAbortedError 优先且 idle 在 cleanup 后收敛 | processor + retry 106/106 通过；完整 Legacy session 504/504 通过 |
| 回归 | explicit incomplete success/exhaustion、clean EOF、ordinary rollback、plugin/tool/error fences | processor + retry 106/106 通过 |
| 回归 | PR #14 context overflow eligible/disabled/assistant-started cases | Prompt + compaction + context-overflow 146 pass / 2 skip / 0 fail |
| E2E | `run-process.test.ts` 全文件，确认 append-only 与 authoritative DB 契约不变 | 20 pass / 0 fail / 131 assertions |
| 全量 | Legacy session suite、LLM schema、opencode/llm typecheck、changed-file lint、`git diff --check` | 全部通过；lint 89 warnings / 0 errors |
...
该 regression 同时锁定 no-next-request、无 summary launch、process 返回 `stop`、内存与 durable assistant 的 `UnknownError` / `finish="error"` / `time.completed`，以及最终 `SessionStatus idle`。

current-HEAD 最终验证（2026-08-21）：
...
| message-ID wraparound | 98 pass / 0 fail / 225 assertions |
...
| opencode / llm typecheck | 均通过 |
| changed-file `oxlint` | 89 warnings / 0 errors（11 files） |
| `git diff --check` | 通过 |

89 项 lint warning 均为 changed files 中既有 unused import、test polling/unsafe assertion 等 warning；本次没有 lint error，也未扩大范围清理无关 warning。
...
| `packages/opencode/src/session/processor.ts` | `beforeRetry` / retry schedule outer cause boundary | 标记 rollback preparation failure、保留 interrupt evidence、把 non-interrupt defect 导入 halt、设置 finish=error | 已改：`57e749efe` |
...
| `packages/opencode/test/session/processor-effect.test.ts` | rollback-removal-failure regression | 增加 terminal error/finish/completed/status/no-next-request 断言 | 已改并通过：`57e749efe` |
| `packages/opencode/test/cli/run/run-process.test.ts` | incomplete CLI E2E | 未新增重复 fixture；processor event-timeline 是 open retained part 的最小可控边界，真实 subprocess 全文件用于跨层回归 | 已重跑：20 pass / 0 fail / 131 assertions |
...
| 文档路径 | 要改什么 | 状态（修复后回填） |
|---------|---------|------------------|
| `docs/fixes/session-fix-incomplete-terminal-settlement.md` | 本八部分方案；修复后回填测试、代码与 commit 状态 | 已回填 P1-A（`49a969975`）、P1-B（`57e749efe`）与 current-HEAD 全量验证 |
| `docs/fixes/session-fix-incomplete-stream-recovery.md` | 补充 rollback preparation fail-closed 的 terminal error/status 后置条件；补充 incomplete terminal `cleanup/message/parts happens-before idle` 契约 | 已补契约并回填 current-HEAD 全量验证 |
| `docs/devlog/2026-08-20-incomplete-stream-recovery.md` | 记录 rebase 后独立审核发现、修复、验证与更新后度量 | 已更新（本验证记录提交） |
| 上层项目 `CLAUDE.md`（nested repo 外） | 增加“terminal idle 不得早于 durable terminal projection；retry preparation defect 必须进入 terminal cause normalization”经验 | 已本地同步（nested repo 外，不进入 PR） |
