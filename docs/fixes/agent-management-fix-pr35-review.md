...
### 0.5 PR #35 上的真实评审（yixiao5428，2026-09-15）—— **两条 P0,我此前都没修好**

这条评审在 PR 上,不在任何 `/tmp` 文档里,我先前几轮**完全没看它**。两条均已用它附带的复现用例
在本分支上实跑核实。

| 编号 | 问题 | 我这边的状态 | 处置 |
|---|---|---|---|
| **PR-P0-1** | depth ≥ 2 的嵌套委托全部正常完成后进程**永不退出** | **未修,且我第三轮的决定正是成因** | 已修（见下） |
| **PR-P0-2** | ceiling 被成员正常运转解除且永不重装 | 已在第三轮修掉（`noteActivity` + 每事件重新求值） | 用其复现用例回归验证通过 |

**PR-P0-1 为什么我没修掉**：第三轮复审（§0.2 B1）指出"子的 idle 早于结果投递,据其退出会丢结果",
我据此把退出**锚死在 root 自己的 idle** 上。那对 depth 1 成立——每个子都向 root 汇报,root 必然最后安静;
**depth ≥ 2 就断了**:B 向 A 汇报,A 跑完转 idle,而 root 早在几步之前就用掉了它最后一次 idle,
没有任何机制会再给它一次。两条评审各自都对,我只满足了其中一条。

**修法：把"等什么"和"等多久"拆成两种**，退出条件与 root 解锚：

- **有人忙但不出声** → 长 ceiling → abort + 如实报告（有工作在被丢掉）。
  该 ceiling 只在 root 已 idle 时武装,故 abort 不会落在正在消费结果的 root 上（评审第 3 点）。
- **全体安静** → 短 settle（1 秒）→ **干净退出**,不报"放弃"。
  settle 的长度依据:它要覆盖的是 deferred 结算 → fork → 父转 busy 这一段**纯进程内**的交接,
  量级是毫秒;1 秒是三个数量级的余量,且每个用到子 Agent 的 run 只付一次。
- **从未创建过子 Session** → 无交接窗口,root 一 idle 即退出,与本 feature 出现之前完全一致。
- 已 abandoned 后不再重装 ceiling,改走 settle——abort 若送不到也不会反复重试。
  这顺带覆盖了此前写不出的"abort 失败仍能结束等待"。

**回归用例**（评审的两个复现,断言方向转正,合入 `run-process.test.ts`）：
