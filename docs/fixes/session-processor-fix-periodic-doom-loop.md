...
- 状态：Issue #20 范围修复完成；已集成到 `dev@1df9f230ad` 之后的组合分支并通过 176/176 当前定向回归；settlement 后完整 lifecycle replay 记录为独立 follow-up；GitHub issue 回复待用户另行确认
- 日期：2026-08-27
...
- 当前集成分支：`context-error-classification`（保留 #19 incomplete-stream recovery 与 untyped context-overflow 修复）
...
| 回归                 | `test/session/processor-effect.test.ts` 全文件             | processor settlement、overflow、permission、tool cleanup 无回归                                                                 | 当前组合分支 76/76 通过，569 次断言                                     |
...
| 真实 CLI E2E         | `test/cli/run/periodic-doom-loop-process.test.ts`          | period 1/2/3/10 正例；两轮、near-miss reset、period 11 负例；配置 ask/allow/deny；核对 SQLite tool parts 与 Provider request 数 | 当前组合分支 10/10 通过，307 次断言；原始端测 commit `93279982e`        |
...
| 定向总回归           | detector + Processor + permission + 两个 process suite     | Issue #20 与当前 processor 基线的组合验证                                                                                       | 当前组合分支 176/176 通过，19,529 次断言                                |
...
集成到 #19 后，真实 AI SDK 工具执行可能在 `doom_loop` permission settlement 之前并发完成。CLI ask/deny E2E
因此继续逐项校验 exact `{callID, tool, input}` 映射，并要求所有 tool parts 最终为 `completed | error`；它不再
要求把已经真实执行完成的调用改写成 `error`。进程退出、permission 证据、assistant terminal error 与 continuation
未消费仍分别由既有断言覆盖。这与第 5.8 节“不回滚已完成工具副作用”的不变量一致，不是放松周期检测条件。
...
| `packages/opencode/test/session/processor-effect.test.ts`             | normalized stream/permission integration                     | period-1/2、唯一 ID、active 重复投递、permission allow/deny/ask、history isolation                | 基础矩阵：`5fc4787dd`；最终有界修正：`d67297528`；当前组合分支 76/76 通过 |
