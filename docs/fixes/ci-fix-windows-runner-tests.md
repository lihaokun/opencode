...
- 状态：断言修正 `fbff2233c0`、调度层修正 `68c7db55c3`、file-search readiness 修正 `cfa91ddecd`、异步测试边界修正 `832d440911`、search 初始化顺序修正 `ecaf44fc70`、Linux lifecycle 修正 `987f4522ee`、Windows scoped handler 修正 `ff21e93abe` 已完成；待下一轮 CI
- 日期：2026-08-19
...
第五轮 CI 中 Linux Unit 在 10 分 15 秒完整通过，证明 worktree Ready 与 scope 修正解决了 6 小时卡死；两组 E2E、typecheck、nix 与规范检查也全部通过。Windows Unit 仍只有 File HttpApi 一项失败，`findText` 在 1.36 秒返回空 503。`test.serial` 和顺序 endpoint 请求没有改变结果，证明问题不在同文件用例调度，而在该文件继续复用 process-global `webHandler`/memoMap；串行标记无法阻止其他文件的 cleanup 影响共享 runtime。

第五轮 run：https://github.com/lihaokun/opencode/actions/runs/32212630051
...
- File HttpApi 测试使用 process-global `HttpApiApp.webHandler()` 和共享 memoMap，同时 suite 中存在显式 concurrent tests 及全局 `disposeAllInstances()` cleanup。Effect HTTP 将被服务端中断的纯 interrupt 映射为空 503；这与 Windows 唯一失败形态一致，而不是 search handler 返回了业务错误。`test.serial` 只改变测试调度，不会为 handler 建立独立 scope，因此第五轮仍复现相同 503。
...
9. File HttpApi 用例迁移到仓库既有的 `testEffect(httpApiLayer)` + `it.instance` 模式。每个用例使用 scoped 真实路由与服务，不再复用 process-global `webHandler`/memoMap；endpoint 请求保持顺序，响应与内容断言不变。
...
- File HttpApi scoped layer 仍执行同一组真实 endpoint 与 12 个断言；变化只在测试资源所有权，每个用例的 handler、服务图和 tmpdir 由同一个 Effect scope 建立并释放，不再受其他文件的 global cleanup 干扰。
...
| 复跑 | E2E 不改断言，以较低 worker 数运行完整 suite | 已改：`68c7db55c3`；第五轮 Linux/Windows 均 93/93 通过 |
...
| 回归 | File HttpApi 不与 process-global instance cleanup 并发 | `987f4522ee` 的 serial 方案第五轮仍失败；scoped layer `ff21e93abe` 本地通过，待 Windows CI |
| 回归 | worktree.create 等待 Ready 后删除，完整 lifecycle 受 timeout | 已改：`987f4522ee`；定向 2/2、严格 5 秒 effect 208/208、本地三阶段 208/208 及第五轮 Linux CI 通过 |
...
- File HttpApi scoped layer 单次 2/2、五个独立进程 10/10、强制并发三轮 6/6 通过；worktree create/invalid 定向 2/2 通过。
...
| `packages/opencode/test/server/httpapi-file.test.ts` | scoped `httpApiLayer` + 顺序请求，移除全局 handler 生命周期竞态 | 已改：`ff21e93abe`，待 CI |
| `packages/opencode/test/server/httpapi-exercise/{index,runner,runtime,types}.ts` | worktree Ready watcher；完整 scope 纳入 timeout | 已改：`987f4522ee`；第五轮 CI 通过 |
| `.github/workflows/test.yml` | HttpApi exerciser step 增加 20 分钟上限 | 已改：`987f4522ee`；第五轮 CI 通过 |
