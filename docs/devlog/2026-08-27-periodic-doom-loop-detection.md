...
| `test/session/processor-effect.test.ts`              | 当前组合分支 76/76 通过，569 次断言                                                               |
| `test/permission/next.test.ts`                       | 79/79 通过，118 次断言                                                                            |
| `test/cli/run/periodic-doom-loop-process.test.ts`    | 当前组合分支 10/10 通过，307 次断言；真实 CLI、SQLite 和本地 Provider HTTP/SSE                    |
...
| Issue #20 与当前 processor 基线的组合定向集          | 176/176 通过，19,529 次断言                                                                       |
...
- 独立端测复审要求 exact cardinality + `{callID, tool, input}` 映射，避免仅比较 ID Set 掩盖 duplicate parts 或 fragment 组装错位；完成路径要求全部 `completed`，停止路径要求全部为 terminal `completed | error`。后者兼容 AI SDK 在 permission settlement 前已并发完成工具的真实状态，不伪造 side-effect rollback。alternating fixture 同时改为相同 input、不同 tool，独立证明 tool 名属于 detector 签名。
...
| 测试通过率     | 原分支定向 147/147；当前组合分支 176/176；spawned E2E 稳定性 36/36；此前全量基线 3383/3392（另 22 skip、1 todo） |
