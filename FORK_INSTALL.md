...
本 fork 需要**从源码构建**并自定义版本号(当前为 `1.18.25-fmv2`),因为:
...
OPENCODE_CHANNEL=dev OPENCODE_VERSION=1.18.25-fmv2 bun run script/build.ts --single
...
验证:`opencode --version` 应显示 `1.18.25-fmv2`。
...
1. **`OPENCODE_VERSION` 环境变量** → 逐字采用(我们就用这个:`1.18.25-fmv2`)；
