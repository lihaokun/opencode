...
import { Parameters as AgentParams } from "../../src/tool/agent"
...
describe("tool parameters", () => {
  describe("JSON Schema (wire shape)", () => {
    ...
    test("agent", () => expect(toJsonSchema(AgentParams)).toMatchSnapshot())
  ...
  describe("agent", () => {
    test("accepts description + prompt + subagent_type", () => {
      const parsed = parse(AgentParams, { description: "d", prompt: "p", subagent_type: "general" })
      expect(parsed.subagent_type).toBe("general")
    })
    test("accepts an optional instance name and working directory", () => {
      const parsed = parse(AgentParams, {
        description: "d",
        prompt: "p",
    ...
    test("rejects missing prompt", () => {
      expect(accepts(AgentParams, { description: "d", subagent_type: "general" })).toBe(false)
    })
  })
