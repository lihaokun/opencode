...
describe("public event manifest", () => {
  test("contains every latest public wire type once", () => {
    ...
    // 89 rather than 88 for agent.delegation. The count is here so that adding a
    // public wire type is a deliberate act, so the new one is named below rather
    // than only counted.
    ...
    expect(EventManifest.Latest.get("agent.delegation")).toBe(AgentEvent.Delegation)
  })
