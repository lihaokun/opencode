...
function expectCalls(parts: Array<Record<string, unknown>>, response: PeriodicToolCallFixture, status: string) {
  expect(parts).toHaveLength(response.calls.length)
  for (const expected of response.calls) {
...
function expectAsk(input: {
  ...
  return Effect.gen(function* () {
    ...
    expectCalls(parts, input.response, "error")
  ...
  cliIt.live(
    ...
    (test) =>
      Effect.gen(function* () {
        ...
        const parts = yield* storedToolParts(test.opencode, sessionID(result, test))
        expectCalls(parts, response, "error")
