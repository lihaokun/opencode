...
| `task`               | Deprecated name for `agent`; still read, renamed on load         |
...
`read`, `edit`, `glob`, `grep`, `list`, `bash`, `agent`, `agent_list`, `agent_send`, `agent_stop`, `external_directory`, `lsp`, and `skill` accept either a shorthand action (`"allow" | "ask" | "deny"`) or an object of glob/pattern → action for fine-grained control. The remaining keys accept the shorthand action only.
...
Control which subagents an agent can invoke via the `agent` tool with `permission.agent`. Uses glob patterns for flexible matching.
...
When set to `deny`, the subagent is removed from the `agent` tool description entirely, so the model won't attempt to invoke it.

The three companion tools take their own keys: `agent_list` for reading the Agent
tree, `agent_send` for messaging another Agent, and `agent_stop` for stopping
one. Denying `agent_list` also withholds the subagent roster and the neighbour
snapshot, so a subagent that cannot list other Agents is not told about them by
another route.

:::note
`permission.task` was this key's name before the tool was renamed. It is still
read and renamed to `agent` on load, keeping its position among your other rules
so an existing `"task": "deny"` goes on denying exactly what it denied before.
Rename it when convenient; it will warn once on load until you do.
:::
...
:::tip
Users can always invoke any subagent directly via the `@` autocomplete menu, even if the agent's `agent` permissions would deny it.
:::
